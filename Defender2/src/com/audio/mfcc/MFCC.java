...
public class MFCC {   
    ...
    public double[] getFilterBankOutputs(double[] fspeechFrame) {   
        ...
        public double[] get_mfcc(byte[] audioData) throws UnsupportedAudioFileException, IOException {   
   
          
   
          //simulate a frame of speech   
         
          //File file = new File("D:\\VOICE\\1\\8.wav");
//          AudioInputStream currentSound = AudioSystem.getAudioInputStream(file); 
//          AudioFormat format = currentSound.getFormat();   
//          byte[] audioData = new byte[((int) currentSound.getFrameLength()*format.getFrameSize())];
//          currentSound.read(audioData);   
          ...
          double [] avresult = new double[12];
          
          while(temp+512<audioData.length)
          { 	  
		          double [] input = new double[512];
		          for(int i=0;i<512;i++){
		        	    input[i]=audioData[temp];
		        	    temp = temp + 1;
		          }
		          double[] dparameters = this.getParameters(input);   
		          for (int i = 0; i < dparameters.length; i++) {   
 		              result[i]=result[i]+dparameters[i];   
			          }   
		          time=time+1;
          }
          for (int i = 0; i < 12; i++) {   
	              avresult[i]+= result[i]/time;   
	      }  
